## Edited Part of code from EasyLM <3
...
class FlaxLGeMConfig(PretrainedConfig):
    ...
    def get_partition_rules(fully_fsdp: bool = True):
        ...
        return (
            ...
            # mlp
            ("feed_forward/gate_proj/kernel", PS("fsdp", "mp")),
            ("feed_forward/down_proj/kernel", PS("mp", "fsdp")),
            ("feed_forward/up_proj/kernel", PS("fsdp", "mp")),
            # layer norms
            ...
            # embeddings
            ("model/wte/embedding", PS("fsdp")),
            # atention
            ("attention/(q_proj|k_proj|v_proj)/kernel", PS("fsdp")),
            ("attention/o_proj/kernel", PS("fsdp")),
            # mlp
            ("feed_forward/gate_proj/kernel", PS("fsdp")),
            ("feed_forward/down_proj/kernel", PS("fsdp")),
            ("feed_forward/up_proj/kernel", PS("fsdp")),
            # layer norms
            ("attention_norm/kernel", PS(None)),
            ("ffn_norm/kernel", PS(None)),
            # output head
...
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0, dtype: jnp.dtype = jnp.bfloat16) -> jnp.ndarray:
    ...
    sin, cos = jnp.sin(freqs), np.cos(freqs)
...
class FlaxLGeMAttention(nn.Module):
...
class FlaxLGeMBlock(nn.Module):
    ...
    def setup(self) -> None:
        self.attention = FlaxLGeMAttention(
            self.config,
            dtype=self.dtype,
            param_dtype=self.param_dtype,
            precision=self.precision,
        )
        self.feed_forward = FlaxLGeMMLP(
            self.config,
            dtype=self.dtype,
            param_dtype=self.param_dtype,
            precision=self.precision,
...
class FlaxLGeMBlockCollection(nn.Module):
    config: FlaxLGeMConfig
    dtype: jnp.dtype = jnp.bfloat16
    param_dtype: jnp.dtype = jnp.bfloat16
    precision: Optional[Union[jax.lax.Precision, str]] = None
...
class FlaxLGeMModule(nn.Module):
    ...
    def setup(self):
        ...
        self.h = FlaxLGeMBlockCollection(self.config, dtype=self.dtype, param_dtype=self.param_dtype,
                                         precision=self.precision)
...
class FlaxLGeMForCausalLMModule(nn.Module):
    ...
    def setup(self):
        self.model = FlaxLGeMModule(self.config, dtype=self.dtype)
