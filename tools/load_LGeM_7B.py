...
from utils.utils import prompt_to_instruction
...
def main():
    ...
    data = load_file('LGeM.safetensors')
    ...
    def load_lgm_weights(model, weights, requires_grad=True, dtype=dtype):

        model_embed_tokens_weight = weights['model.embed_tokens.weight'].to(dtype)
        model.model.embed_tokens.weight = torch.nn.Parameter(model_embed_tokens_weight)
        pb = tqdm(enumerate(model.model.layers), total=len(model.model.layers))
        for i, layer in pb:

            layer.self_attn.q_proj.weight = torch.nn.Parameter(
                weights[f'model.layers.{i}.self_attn.q_proj.weight'].to(dtype),
                requires_grad=requires_grad)
            layer.self_attn.k_proj.weight = torch.nn.Parameter(
                weights[f'model.layers.{i}.self_attn.k_proj.weight'].to(dtype),
                requires_grad=requires_grad)
            layer.self_attn.v_proj.weight = torch.nn.Parameter(
                weights[f'model.layers.{i}.self_attn.v_proj.weight'].to(dtype),
                requires_grad=requires_grad)
            layer.self_attn.o_proj.weight = torch.nn.Parameter(
                weights[f'model.layers.{i}.self_attn.o_proj.weight'].to(dtype),
                requires_grad=requires_grad)
            if hasattr(layer.self_attn, 'rotary_emb'):
                layer.self_attn.rotary_emb.inv_freq = torch.nn.Parameter(
                    weights[f'model.layers.{i}.self_attn.rotary_emb.inv_freq'].to(dtype), requires_grad=requires_grad)
            layer.mlp.down_proj.weight = torch.nn.Parameter(weights[f'model.layers.{i}.mlp.down_proj.weight'].to(dtype),
                                                            requires_grad=requires_grad)
            layer.mlp.gate_proj.weight = torch.nn.Parameter(weights[f'model.layers.{i}.mlp.gate_proj.weight'].to(dtype),
                                                            requires_grad=requires_grad)
            layer.mlp.up_proj.weight = torch.nn.Parameter(weights[f'model.layers.{i}.mlp.up_proj.weight'].to(dtype),
                                                          requires_grad=requires_grad)
            layer.input_layernorm.weight = torch.nn.Parameter(
                weights[f'model.layers.{i}.input_layernorm.weight'].to(dtype),
                requires_grad=requires_grad)
            layer.post_attention_layernorm.weight = torch.nn.Parameter(
                weights[f'model.layers.{i}.post_attention_layernorm.weight'].to(dtype), requires_grad=requires_grad)
        model.model.norm.weight = torch.nn.Parameter(weights[f'model.norm.weight'].to(dtype))
        model.lm_head.weight = torch.nn.Parameter(weights[f'lm_head.weight'].to(dtype))
        return model

    model = load_lgm_weights(model, weights=data)
    logger.info('Model Loaded')
    prompt = prompt_to_instruction('describe an Atom to me')
    encoded = tokenizer(prompt, return_tensors='pt')

    pred = model.forward(encoded['input_ids'])
    print(pred)
